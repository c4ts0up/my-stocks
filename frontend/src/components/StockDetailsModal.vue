<template>
  <div v-if="stock" class="fixed inset-0 bg-gray-300 bg-opacity-75 flex items-center justify-center p-4">
    <div class="bg-white p-6 rounded-lg shadow-xl max-w-[800px] w-full border border-gray-300 text-center">
      <h3 class="mb-1 text-gray-800">{{ stock.stock_base.ticker }}</h3>
      <h2 class="mb-1 text-2xl font-semibold text-gray-900 title-font">{{ stock.stock_base.company_name }}</h2>
      <h3 class="mb-4 text-gray-800">{{ stock.stock_base.last_price.toFixed(2) }} $</h3>

      <span :class="getRecommendationClass(stock.stock_base.recommendation, SIZE.xl)">
